...
void render(GLFWwindow* window)
{
	...
	if( !m )
	{
		mjr_rectangle(1, rect, 0, 0, rect.width, rect.height, 
			0.2, 0.3, 0.4, 1);
		mjr_overlay(rect, mjGRID_TOPLEFT, 0, "No model to plot", 0, &con);
		gui_mutex.unlock();
		return;
	}
	/*
...
std::thread * rendering_th = 0;
bool isVizualizer = false;


int visualize_render()
{
	GLFWwindow* window = 0;
	...
	while (isVizualizer)
    {
        ...
        // sleep ??
        while( glfwGetTime()-lasttm<0.008 )
	...
	glfwTerminate();
	mexPrintf("Visualization exiting\n");
	return 0;
}

void visualize_close()
{
	// signal thread for exit 
	isVizualizer = false;

	// wait for thread
	if (rendering_th && rendering_th->joinable())
	{
		rendering_th->join();
		rendering_th->~thread();
		mju_MATLAB_free(rendering_th);
		rendering_th = 0;
	}
}
...
int visualize_init()
{
	...
	if (!isVizualizer)
	{
		//std::thread th1(visualize_render);
		//th1.detach();
		rendering_th = (std::thread*)mju_MATLAB_malloc(sizeof(std::thread));
		rendering_th = new(rendering_th) std::thread(visualize_render);

		isVizualizer = true;
		mexPrintf("Visualization started\n");
	}
	return 0;
}