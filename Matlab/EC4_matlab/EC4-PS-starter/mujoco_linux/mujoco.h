...
// check license, return code is of type mjtLicense
MJAPI int mj_license(const char* filename);
...
MJAPI void mj_objectVelocity(const mjModel* m, const mjData* d, 
                             int objtype, int objid, mjtNum* res, mjtByte flg_local);
...
// version number: 1.0.2 is encoded as 1.02
MJAPI double mj_version(void);
...
// text overlay
MJAPI void mjr_overlay(mjrRect viewport, mjtGridPos gridpos, int big,
                       const char* overlay, const char* overlay2, const mjrContext* con);

// draw rectangle
MJAPI void mjr_rectangle(int flg_init2d, mjrRect viewport,
...
// (re) upload texture to GPU: both 2d and cube versions (if size if valid for cube)
...
MJAPI void mju_transformSpatial(mjtNum* res, const mjtNum* vec, mjtByte flg_force,
